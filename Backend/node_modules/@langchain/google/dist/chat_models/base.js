...
function hasVertexCredentials(params) {
	return typeof params.credentials !== "undefined" || typeof params.googleAuthOptions !== "undefined" || typeof getEnvironmentVariable("GOOGLE_CLOUD_CREDENTIALS") !== "undefined";
}
/**
...
function applyGeminiGatewayParams(params) {
	...
	if (convertParamsToPlatformType(params) === "gcp" || hasVertexCredentials(params)) return { ...params };
	const gatewayConfig = resolveLangSmithGatewayConfig({ providerPath: "gemini" });
...
var BaseChatGoogle = class extends BaseChatModel {
	...
	*/
	...
	async _generate(messages, options, runManager) {
		...
		try {
			response = await this.caller.callWithOptions({
				...
				if (!nextResponse.ok) throw await RequestError.fromResponse(nextResponse);
				return nextResponse;
			});
		} catch (error) {
	...
	async *_streamChatModelEvents(messages, options, _runManager) {
		...
		const eventStream = response.body.pipeThrough(new TextDecoderStream()).pipeThrough(new EventSourceParserStream()).pipeThrough(new SafeJsonEventParserStream());
		...
		yield* convertGoogleGeminiStream(geminiChunks(eventStream, options.signal), { streamUsage: shouldStreamUsage });
	}
...
//#endregion
export { BaseChatGoogle, applyGeminiGatewayParams, combineGoogleChatModelFields, getGoogleChatModelParams, getPlatformType };

...
const MEDIA_BLOCK_TYPES = /* @__PURE__ */ new Set([
	"media",
	"image",
	"audio",
	"video",
...
function isMediaContentBlock(item) {
	if (typeof item !== "object" || item === null) return false;
	if (isDataContentBlock(item)) return item.source_type !== "text";
	if ("image_url" in item || "inlineData" in item || "fileData" in item) return true;
	return MEDIA_BLOCK_TYPES.has(item.type);
}
function stripMediaBlocksForFunctionResponse(content) {
...
function convertStandardContentMessageToGeminiContent(message, messages) {
	...
	if (ToolMessage.isInstance(message) && message.tool_call_id) {
		const responseContent = stripMediaBlocksForFunctionResponse(message.content);
	...
	*/
	...
	if (typeof message.content === "string") {
		...
		for (const item of message.content) if (typeof item === "string") parts.push({ text: item });
			...
			if (isMessageContentText(item)) parts.push({ text: item.text });
	...
	if (ToolMessage.isInstance(message) && message.tool_call_id) {
		...
		parts = parts.filter((part) => "functionResponse" in part || "inlineData" in part || "fileData" in part);
	}
...
*/
function canMergeInto(prev, content) {
	return prev !== void 0 && prev.role === content.role && hasFunctionResponse(prev) === hasFunctionResponse(content);
}
/**
...
//#endregion
export { convertAIMessageToText, convertGeminiCandidateToAIMessage, convertGeminiGenerateContentResponseToUsageMetadata, convertGeminiPartToContentBlock, convertGeminiPartsToToolCalls, convertMessagesToGeminiContents, convertMessagesToGeminiSystemInstruction, geminiContentBlockConverter };

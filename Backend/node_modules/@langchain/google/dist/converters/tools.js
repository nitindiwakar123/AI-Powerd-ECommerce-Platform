...
const convertToolsToGeminiTools = (tools) => {
	...
	const geminiTools = [];
	const functionDeclarations = [];
	for (const tool of tools) if (isGeminiTool(tool)) {
		if ("functionDeclarations" in tool) geminiTools.push(tool);
...
function convertToolChoiceToGeminiConfig(toolChoice, hasTools) {
	...
	if (toolChoiceFunction) {
		if (Array.isArray(toolChoiceFunction)) allowedFunctionNames = toolChoiceFunction;
...
//#endregion
export { convertToolChoiceToGeminiConfig, convertToolsToGeminiFunctionDeclarations, convertToolsToGeminiTools, schemaToGeminiParameters };
